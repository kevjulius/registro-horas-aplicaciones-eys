...
npm.cmd run build
Start-Process -FilePath npm.cmd -ArgumentList "run", "start" -WorkingDirectory $root.Path -WindowStyle Hidden
Start-Sleep -Seconds 4

try {
  $response = Invoke-WebRequest -Uri "http://localhost:3000" -UseBasicParsing -TimeoutSec 10
  Write-Host "Servidor estable iniciado en http://localhost:3000 - Status $($response.StatusCode)"
} catch {
